@tool
class_name XRToolsMovementMultiplayerDirect
extends XRToolsMovementProviderMultiplayer
...
func physics_movement(_delta: float, player_body: XRToolsPlayerBodyMultiplayer, _disabled: bool):
...
static func find_left(node : Node) -> XRToolsMovementMultiplayerDirect:
	return XRTools.find_xr_child(
		XRHelpers.get_left_controller(node),
		"*",
		"XRToolsMovementDirect") as XRToolsMovementMultiplayerDirect
...
## [XRToolsMovementDirect] assuming the node is a sibling of the [XROrigin3D].
static func find_right(node : Node) -> XRToolsMovementMultiplayerDirect:
	return XRTools.find_xr_child(
